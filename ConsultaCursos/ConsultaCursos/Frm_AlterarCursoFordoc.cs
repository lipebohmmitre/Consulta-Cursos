...
{
    public partial class Frm_AlterarCursoFordoc : Form
    {
        ...
        private void Btn_procurarCursoAlterar_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                var buscaLike = new MySqlCommand($"SELECT id_cursos, nome_curso, nome_periodo, carga_horaria FROM tb_cursos A INNER JOIN tb_periodo B ON A.id_periodo = B.id_periodo WHERE nome_curso = '{Txt_procurarCurso.Text}' AND A.id_periodo = 1", conexao);
        ...
        private void Btn_alterar_Click(object sender, EventArgs e)
        {
            try
            {
                if (Txb_nomeCursoAlterar.Text != "" && Txb_cargaHorariaAlterar.Text != "")
                {
                    ...
                    var consultaCursoExistente = new MySqlCommand($"SELECT COUNT(1) FROM tb_cursos WHERE nome_curso = '{Txb_nomeCursoAlterar.Text}' AND id_periodo = 1;", conexao);
                    ...
                    if (result != null)
                    {
                        ...
                        {
                            var updateCursoPos = new MySqlCommand($"UPDATE tb_cursos SET nome_curso = '{Txb_nomeCursoAlterar.Text}', carga_horaria = {Txb_cargaHorariaAlterar.Text} WHERE id_cursos = {id} AND id_periodo = 1;", conexao);
