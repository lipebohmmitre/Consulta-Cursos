...
{
    public partial class Frm_ExlcuirCursoSegLic : Form
    {
        ...
        private void Btn_procurar_Click_1(object sender, EventArgs e)
        {
            try
            {
                ...
                var buscaLike = new MySqlCommand($"SELECT id_cursos, nome_curso, nome_periodo, carga_horaria FROM tb_cursos A INNER JOIN tb_periodo B ON A.id_periodo = B.id_periodo WHERE nome_curso = '{Txb_procurarCurso.Text}' AND A.id_periodo = 3", conexao);
